#' Compute a c-optimal approximate design on a finite candidate set
#'
#' @param u Candidate design points.
#' @param f Regression function returning a numeric vector.
#' @param cVec Numeric vector specifying the linear combination c^T theta.
...
#'
#' @return An object of class "cvx_c_design" and "cvx_design".
#' @export
calc_copt <- function(u,
  ...
  class(out) <- c("cvx_c_design", "cvx_design")
  out
}
