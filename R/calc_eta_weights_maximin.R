...
#' Compute derivatives of the maximin right-hand sides
#'
#' @param tstar Optimal maximin scalar.
#' @param loss_ref Named list of reference losses.
#' @param criteria Character vector of criteria.
...
#' Compute eta weights for the maximin equivalence theorem
#'
...
#'
#' This function is the R-package counterpart of the MATLAB routine
#' `calc_eta_weights()`.
#'
...
calc_eta_weights_maximin <- function(tstar,
  ...
  # Normalization: sum_j eta_j h'_j(tstar) = 1
  norm_expr <- 0
  for (k in seq_len(K)) {
  ...
  # Complementary slackness approximation: eta_j * gap_j ≈ 0
  for (k in seq_len(K)) {
    key <- canon_crit_key(criteria[k])
    delta <- gaps[[key]]
  ...
  # Weighted combined directional derivative <= tol pointwise
  sum_deriv <- 0
  for (k in seq_len(K)) {
    field <- dd_names[k]
