...
where $y_i$ is the response observed at design point $\mathbf{x}_i \in \mathcal{X}$, $\boldsymbol{\theta} \in \mathbb{R}^p$ is an unknown parameter vector, and $\eta(\mathbf{x}, \boldsymbol{\theta})$ is the mean response function, which may be linear or nonlinear in the parameters.
